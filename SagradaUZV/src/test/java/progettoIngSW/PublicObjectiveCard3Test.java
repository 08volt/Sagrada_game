...
public class PublicObjectiveCard3Test {
    ...
    @Test
    public void checkPublicCard3() throws  IOException {
        Pattern p = new Pattern(0);
        WindowFrame wf = new WindowFrame(p);
        //Calcolo punteggio su wf vuota
        PublicObjectiveCard3 card0 = new PublicObjectiveCard3();
        assertEquals("Calcolo punteggio sbagliato", 0, card0.scoringCalc(wf));
        ...
        //Posizionati alcuni dadi sparsi (nessuna riga completata per il calcolo del punteggio)
        PublicObjectiveCard3 card00 = new PublicObjectiveCard3();
        assertEquals("Calcolo punteggio sbagliato", 0, card00.scoringCalc(wf));
        ...
        PublicObjectiveCard3 card3 = new PublicObjectiveCard3();
        assertEquals("Calcolo punteggio sbagliato", 0, card3.scoringCalc(wf));
        wf.getCell(0, 0).getDice().setNumber(6);
        assertEquals("Calcolo punteggio sbagliato", 5, card3.scoringCalc(wf));
        try{
            wf.removeDice(0);
